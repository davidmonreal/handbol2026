...
describe('Integration Tests: Match Event Flow', () => {
    ...
    it('should calculate match score correctly from events', async () => {
        ...
        const homeGoals = events.filter((e: any) =>
            e.type === 'Shot' && e.subtype === 'Goal' && e.teamId === match?.homeTeamId
        ).length;

        const awayGoals = events.filter((e: any) =>
            e.type === 'Shot' && e.subtype === 'Goal' && e.teamId === match?.awayTeamId
        ).length;
        ...
        const totalGoalEvents = events.filter((e: any) =>
            e.type === 'Shot' && e.subtype === 'Goal'
        ).length;
