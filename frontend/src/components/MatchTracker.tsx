...
import { Play, Pause, RotateCcw, Plus, Minus, Users, Target, Activity, AlertTriangle, FileWarning, Shield, Zap } from 'lucide-react';
...
const MatchTracker = () => {
  ...
  const handleFinalizeEvent = (targetIndex?: number) => {
        ...
        <div className="lg:col-span-8">
          {selectedPlayerId ? (
            <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-6">
                        <button
                          ...
                          className={`p-3 rounded-lg font-medium border-2 transition-colors text-xs md:text-sm ${
                            isCollective ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`p-3 rounded-lg font-medium border-2 transition-colors text-xs md:text-sm ${
                            hasOpposition ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-200 text-gray-500'
                          }`}
                        >
                          {hasOpposition ? 'Opposition' : 'Free'}
                        </button>
                         <button
                          onClick={() => setIsCounterAttack(!isCounterAttack)}
                          className={`p-3 rounded-lg font-medium border-2 transition-colors text-xs md:text-sm flex items-center justify-center gap-1 ${
                            isCounterAttack ? 'border-yellow-500 bg-yellow-50 text-yellow-700' : 'border-gray-200 text-gray-500'
                          }`}
                        >
                          <Zap size={14} />
                          {isCounterAttack ? 'Counter' : 'Static'}
                        </button>
                      </div>
                      ...
                      {selectedAction && (
                        <div className="animate-fade-in">
                          ...
                          <div className="max-w-[200px] mx-auto aspect-square bg-gray-100 rounded-lg p-2">
                            <div className="grid grid-cols-3 grid-rows-3 gap-2 h-full">
                              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((target) => (
                                <button
                                  key={target}
                                  onClick={() => handleFinalizeEvent(target)}
                                  className="bg-white border hover:bg-indigo-50 hover:border-indigo-300 rounded transition-colors"
                                />
                              ))}
                            </div>
                          </div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-4 md:p-6 animate-fade-in">
                  ...
                  {selectedAction && (
                    <div className="mt-6 animate-fade-in">
                       ...
                       <div className="grid grid-cols-3 gap-2">
                          <button onClick={() => { setSelectedZone('9m-CB'); handleFinalizeEvent(); }} className="p-3 bg-gray-100 hover:bg-gray-200 rounded text-sm">9m Area</button>
                          <button onClick={() => { setSelectedZone('6m-CB'); handleFinalizeEvent(); }} className="p-3 bg-gray-100 hover:bg-gray-200 rounded text-sm">6m Area</button>
              ...
              {flowType === 'Sanction' && (
                <div className="bg-white rounded-xl shadow-lg p-4 md:p-6 animate-fade-in">
                  <h3 className="text-lg font-bold text-gray-800 mb-4">2. Select Sanction</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => { setSelectedAction('Yellow Card'); handleFinalizeEvent(); }} className="p-4 bg-yellow-400 text-yellow-900 font-bold rounded-lg hover:opacity-90">Yellow Card</button>
                    <button onClick={() => { setSelectedAction('2 Minutes'); handleFinalizeEvent(); }} className="p-4 bg-gray-800 text-white font-bold rounded-lg hover:opacity-90">2 Minutes</button>
                    <button onClick={() => { setSelectedAction('Red Card'); handleFinalizeEvent(); }} className="p-4 bg-red-600 text-white font-bold rounded-lg hover:opacity-90">Red Card</button>
                    <button onClick={() => { setSelectedAction('Blue Card'); handleFinalizeEvent(); }} className="p-4 bg-blue-600 text-white font-bold rounded-lg hover:opacity-90">Blue Card</button>
                  </div>
                </div>
              )}
