...
describe('Goalkeeper Flow Integration', () => {
    ...
    it('Complete Goalkeeper Flow: Create -> Record Event -> Verify Stats', async () => {
        // 1. Create Goalkeeper
        const gkData = { name: 'Nico', number: 1, handedness: 'RIGHT', isGoalkeeper: true };
        const createdGk = { id: 'gk-1', ...gkData };

        vi.mocked(playerService.create).mockResolvedValue(createdGk as any);

        // Simulate Controller Call
        const req = { body: gkData } as any;
        ...
        vi.mocked(gameEventRepository.create).mockResolvedValue(saveEvent as any);
        ...
        expect(gameEventRepository.create).toHaveBeenCalledWith(expect.objectContaining({
            activeGoalkeeperId: 'gk-1',
            subtype: 'Save'
        }));
        ...
        vi.mocked(gameEventRepository.findByMatchId).mockResolvedValue(matchEvents as any);
        ...
        expect((retrievedEvents[0] as any).activeGoalkeeperId).toBe('gk-1');
        expect((retrievedEvents[1] as any).activeGoalkeeperId).toBe('gk-1');
