...
describe('Full Application Lifecycle Integration Test', () => {
    ...
    const cleanup = async () => {
        ...
        await prisma.playerTeamSeason.deleteMany({ where: { teamId: { in: [homeTeamId, awayTeamId] } } });
    ...
    describe('1. Club Management', () => {
        it('should create a new club', async () => {
            const res = await request(app)
                .post('/api/clubs')
                .send({ name: 'Integration Test Club' });
    ...
    describe('4. Player Management', () => {
        ...
        it('should assign player to home team', async () => {
            const res = await request(app)
                .post(`/api/teams/${homeTeamId}/players`)
                .send({
                    playerId,
                    role: 'Player',
                });
            ...
            const playerInTeam = teamRes.body.players.find((p: any) => p.player.id === playerId);
    ...
    describe('6. Game Events & Statistics', () => {
        ...
        it('should verify match statistics and data integrity', async () => {
            ...
            const goalEvent = events.find((e: any) => e.type === 'Shot');
